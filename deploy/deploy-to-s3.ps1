# MRP Angular - Deploy to S3
# Bucket: si2-mrp-fe
# Region: us-east-1

$BucketName = "si2-mrp-fe"
$BuildFolder = "dist/erp-fe-sakai/browser"
$Region = "us-east-1"

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "  MRP Angular - Deploy S3" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# Assets estaticos (1 año cache)
aws s3 sync $BuildFolder s3://$BucketName --region $Region --delete --cache-control "public,max-age=31536000" --exclude "*.html" --exclude "*.json"
...
aws s3 sync $BuildFolder s3://$BucketName --region $Region --exclude "*" --include "*.html" --include "*.json" --cache-control "public,max-age=0,must-revalidate"
